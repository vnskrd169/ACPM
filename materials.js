// ═══════════════════════════════════════════════════════════════
//  ACPM v4 — materials.js
//  PO builder (with size field) · Ledger · PO History archive
//  Mark all per PO · Materials summary (by item+size)
//  Budget warnings · PNG export
// ═══════════════════════════════════════════════════════════════
...
function watchMatBudget(pid){
  listen(firebase.database().ref(`projects/${pid}`), snap=>{
    ...
    setText('mbBudget',peso(budget)); setText('mbSpent',peso(spent));
    const el=$('mbLeft');
    ...
    if (wb){
      ...
      wb.textContent=p>=95?`⚠ CRITICAL — Materials budget ${p}% used! Only ${peso(left)} left.`:`⚠ WARNING — Materials budget ${p}% used. ${peso(left)} remaining.`;
    }
  });
}
...
function addDraftItem(){
  const desc=document.getElementById('poItemDesc')?.value.trim();
  const size=document.getElementById('poItemSize')?.value.trim()||'';
  const qty =parseFloat(document.getElementById('poItemQty')?.value)||0;
  const unit=document.getElementById('poItemUnit')?.value.trim()||'';
  const cost=parseFloat(document.getElementById('poItemCost')?.value)||0;
  ...
  ['poItemDesc','poItemSize','poItemQty','poItemUnit','poItemCost'].forEach(id=>{ const e=$(id); if(e)e.value=''; });
  renderDraft();
}
...
function renderDraft(){
  ...
  if (!_draftItems.length){ el.innerHTML='<p class="empty-hint">No items added yet.</p>'; setText('draftTotal',peso(0)); return; }
...
async function submitPO(){
  if (!_mpid) return;
  if (!_draftItems.length){ alert('Add at least one item.'); return; }
  ...
  const po={supplier,date,notes,items:_draftItems,total,status:'ordered',createdAt:Date.now(),createdDate:new Date().toLocaleDateString('en-PH')};
  ...
  _draftItems.forEach((item,i)=>{
    ledgerUpdates[`${poRef.key}_${i}`]={
      poId:poRef.key, supplier, date,
      desc:item.desc, size:item.size||'', qty:item.qty, unit:item.unit,
      cost:item.cost, total:item.total,
      status:'ordered', createdAt:Date.now()
    };
  });
  ...
  renderDraft();
  alert(`✅ PO submitted to ${supplier}!`);
}

// ── Quick-fill supplier from directory ───────────────────────
function fillSupplierFromDirectory(name){
...
function watchLedger(pid){
  listen(firebase.database().ref(`projects/${pid}/ledger`), snap=>{
    ...
    if (!snap.exists()){
      tbody.innerHTML=`<tr><td colspan="9" class="empty-cell">No items yet.</td></tr>`;
      setText('ledgerTotal',peso(0)); setText('ledgerCount','0 items');
    ...
    snap.forEach(c=>{
      ...
      const statusOpts=['ordered','delivered','paid','cancelled'].map(s=>`<option value="${s}" ${m.status===s?'selected':''}>${s.charAt(0).toUpperCase()+s.slice(1)}</option>`).join('');
...
// ══════════════════════════════════════════════════════
//  MATERIALS SUMMARY — grouped by item name + size
// ══════════════════════════════════════════════════════
function updateMaterialsSummary(snap){
  ...
  el.innerHTML=`<table class="summary-table">
    <thead><tr>
      <th>Item</th><th>Size</th><th>Total Qty</th><th>Unit</th><th class="s-right">Total Cost</th><th class="s-right">Orders</th>
    </tr></thead>
    <tbody>
...
// ══════════════════════════════════════════════════════
//  PO HISTORY + MARK ALL + PNG EXPORT
// ══════════════════════════════════════════════════════
function watchPOHistory(pid){
  listen(firebase.database().ref(`projects/${pid}/purchaseOrders`), snap=>{
    ...
    container.innerHTML='';
    if (!snap.exists()){ container.innerHTML='<p class="empty-hint" style="padding:20px">No purchase orders yet.</p>'; return; }
    ...
    entries.forEach(po=>{
      ...
      container.innerHTML+=`<div class="po-card" id="poc_${po.id}">
        <div class="po-card-hdr">
          ...
          <div class="po-card-right">
            ...
            <div class="po-btns">
              <button class="po-mark-btn" onclick="markAllPO('${po.id}','delivered')">✓ All Delivered</button>
              <button class="po-mark-btn po-paid-btn" onclick="markAllPO('${po.id}','paid')">✓ All Paid</button>
              <button class="po-export-btn" onclick="exportPOImage('${po.id}')">📷 Save Image</button>
            </div>
          </div>
        </div>
        <div class="po-item-hdr"><span>Item</span><span>Qty</span><span>Unit Cost</span><span>Total</span></div>
        ${itemRows}
      </div>`;
    });
  });
...
async function exportPOImage(poId){
  ...
  wrap.style.cssText=`position:fixed;left:-9999px;top:0;width:620px;background:#ffffff;color:#111;font-family:'Segoe UI',Arial,sans-serif;font-size:13px;padding:36px;box-sizing:border-box;`;

  const itemsHTML=(po.items||[]).map(it=>`<tr>
    <td style="padding:7px 8px;border-bottom:1px solid #e5e7eb">${it.desc}${it.size?` <em style="color:#6b7280;font-size:11px">[${it.size}]</em>`:''}</td>
  ...
  wrap.innerHTML=`
    <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:22px">
      <div>
        ...
        <div style="font-size:24px;font-weight:900">${_mpid}</div>
      </div>
      <div style="text-align:right">
        <div style="font-size:10px;color:#6b7280">Date</div>
        <div style="font-weight:700;font-size:15px">${po.date}</div>
      </div>
    </div>
    <div style="background:#f3f4f6;border-radius:10px;padding:14px 18px;margin-bottom:22px">
      <div style="font-size:10px;color:#6b7280;font-weight:700;margin-bottom:3px">SUPPLIER</div>
      <div style="font-size:18px;font-weight:900">${po.supplier}</div>
      ${po.notes?`<div style="font-size:12px;color:#6b7280;margin-top:4px">${po.notes}</div>`:''}
    </div>
    <table style="width:100%;border-collapse:collapse;margin-bottom:16px">
      <thead><tr style="background:#1e293b;color:#fff">
        <th style="padding:9px 8px;text-align:left;font-size:11px">ITEM / SIZE</th>
        <th style="padding:9px 8px;text-align:center;font-size:11px">QTY</th>
        <th style="padding:9px 8px;text-align:right;font-size:11px">UNIT COST</th>
        <th style="padding:9px 8px;text-align:right;font-size:11px">TOTAL</th>
      </tr></thead>
      <tbody>${itemsHTML}</tbody>
    </table>
    <div style="display:flex;justify-content:flex-end">
      <div style="background:#0f172a;color:#fff;border-radius:10px;padding:14px 22px;text-align:right">
        <div style="font-size:10px;color:#94a3b8;font-weight:600;margin-bottom:3px">TOTAL AMOUNT</div>
        <div style="font-size:26px;font-weight:900">${peso(po.total)}</div>
      </div>
    </div>
    <div style="margin-top:22px;padding-top:12px;border-top:1px solid #e5e7eb;font-size:10px;color:#9ca3af;text-align:center">
      ACPM · Art and Choi Project Management · ${new Date().toLocaleDateString('en-PH')}
    </div>`;
