        ...
        let portfolio = JSON.parse(localStorage.getItem('wld_portfolio')) || {
            "Wolflink HQ": {
                laborBudget: 250000,
                scopes: [
                    { name: "General Workers", leader: "Arnel Delgado", bank: "GCash" }
                ],
                roster: [
                    { id: 1, name: "Dave", rate: 900, group: "General Workers", cashAdvances: [], carriedBalance: 0 }
                ],
                ledgerHistory: []
            }
        };
        ...
        function savePortfolioToStorage() {
            localStorage.setItem('wld_portfolio', JSON.stringify(portfolio));
        }
        ...
        // --- CASH ADVANCE ENGINE WITH FIXED CLICK BINDINGS ---
        ...
        function switchToGlobalHub() {
            ...
            document.getElementById('subHeaderContext').innerText = "Global Multi-Site Portal Hub";
        ...
        function addNewScope() {
            ...
            savePortfolioToStorage(); renderScopeControls(); renderAllGroupTables();
        }
        ...
        function addNewWorkerToRoster() {
            ...
            savePortfolioToStorage(); renderRosterSection(); renderAllGroupTables();
        }
        ...
        function processWeeklyTimecard() {
            ...
            // Temp container array to cleanly compute and apply carried balances afterward
            ...
            if(currentEditingRecordId) {
                ...
                // Commit the new rolling balances and clear out this week's processed cash advance slips
                balanceUpdatesToCommit.forEach(update => {
                    ...
                    if (targetWorker) {
                        ...
                        targetWorker.cashAdvances = []; // clear entries for fresh week ahead
                    }
                });
            }
            ...
            savePortfolioToStorage();
            updateBudgetSpeedometers();
            renderAllGroupTables(); // rebuild table graphics instantly to reveal any new carried adjustments
            document.getElementById('outputContainer').classList.remove('hidden');
            document.getElementById('outputContainer').scrollIntoView({ behavior: 'smooth' });
        }
        ...
        function wipeActiveProjectLogs() {
            if(confirm("Wipe sheet logs for this project?")) {
                portfolio[activeProjectKey].ledgerHistory = []; savePortfolioToStorage(); buildProjectHistoryIndex(); updateBudgetSpeedometers();
            }
        }
