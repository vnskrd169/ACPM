...
function main() {
  ...
  assert(JSON.stringify(localStyles(loginHtml)) === JSON.stringify(['style.css?v=112']), 'login.html must include versioned style.css?v=112');
  ...
  assert(sw.includes("const CACHE_NAME = 'acpm-v141'"), 'service worker cache must be acpm-v141');
  ...
  assert(JSON.stringify(baselineStyles) === JSON.stringify(['style.css?v=112', 'assets/brand/ai-command-center.css?v=1']), 'dashboard.html must include current versioned styles');
