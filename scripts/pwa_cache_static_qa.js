...
function main() {
  ...
  assert(JSON.stringify(localStyles(loginHtml)) === JSON.stringify(['style.css?v=103']), 'login.html must include versioned style.css?v=103');
  ...
  assert(JSON.stringify(localScripts(loginHtml)) === JSON.stringify(['utils.js?v=84', 'auth.js?v=95', 'main.js?v=101']), 'login.html must include current auth shell scripts');
  assert(sw.includes("const CACHE_NAME = 'acpm-v124'"), 'service worker cache must be acpm-v124');
  ...
  assert(JSON.stringify(baselineStyles) === JSON.stringify(['style.css?v=103']), 'index.html must include versioned style.css?v=103');
