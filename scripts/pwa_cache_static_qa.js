...
function main() {
  ...
  assert(JSON.stringify(localStyles(loginHtml)) === JSON.stringify(['style.css?v=111']), 'login.html must include versioned style.css?v=111');
  ...
  assert(sw.includes("const CACHE_NAME = 'acpm-v138'"), 'service worker cache must be acpm-v138');
  ...
  assert(JSON.stringify(baselineStyles) === JSON.stringify(['style.css?v=111']), 'dashboard.html must include versioned style.css?v=111');
