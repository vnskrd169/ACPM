...
function main() {
  ...
  assert(/const CACHE_NAME = 'acpm-v142'/.test(sw), 'service worker cache must be acpm-v142');
  assert(loginHtml.includes('<link rel="stylesheet" href="style.css?v=113">'), 'login.html must load style.css?v=113');
  ...
  assert(extractScriptVersion(loginHtml, 'main.js') === '110', 'login.html must load main.js?v=110');
  ...
  for (const [file, content] of Object.entries(html)) {
    assert(content.includes('<link rel="stylesheet" href="style.css?v=113">'), `${file} must load style.css?v=113`);
    ...
    assert(extractScriptVersion(content, 'main.js') === '110', `${file} must load main.js?v=110`);
    ...
    assert(extractScriptVersion(content, 'labor.js') === '99', `${file} must load labor.js?v=99`);
  ...
  assert(sw.includes('./style.css?v=113'), 'service worker must cache style.css?v=113');
  assert(sw.includes('./auth.js?v=98'), 'service worker must cache auth.js?v=98');
  assert(sw.includes('./main.js?v=110'), 'service worker must cache main.js?v=110');
  assert(sw.includes('./payroll-math.js?v=2'), 'service worker must cache payroll-math.js?v=2');
  assert(sw.includes('./labor.js?v=99'), 'service worker must cache labor.js?v=99');
  ...
  console.log(JSON.stringify({
    result: 'PASS',
    checks: [
      'current cache/style and script references',
