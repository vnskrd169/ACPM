//═══════════════════════════════════════════════════════════════
//  ACPM v6 — main.js  (FIXED)
//  · Comparison bar now lives INSIDE Firebase listener → always live
//  · createProject budget bug fixed (was laborBudget:mb, now lb)
//  · Toast replaces all alerts
//  · Suppliers are global (no pid needed)
// ═══════════════════════════════════════════════════════════════
...
let currentProjectId = null, currentProjectLocked = true, _listeners = [];
...
function showToast(msg, type='success'){
  let t=$('acpm-toast');
  if(!t){ t=document.createElement('div'); t.id='acpm-toast';
    t.style.cssText='position:fixed;bottom:80px;left:50%;transform:translateX(-50%);padding:12px 22px;border-radius:12px;font-size:13px;font-weight:700;z-index:9999;transition:opacity .4s;pointer-events:none;max-width:88vw;text-align:center';
    document.body.appendChild(t); }
  const c={success:{bg:'#064e3b',color:'#6ee7b7',border:'#10b981'},error:{bg:'#7f1d1d',color:'#fca5a5',border:'#ef4444'},warn:{bg:'#451a03',color:'#fcd34d',border:'#f59e0b'}}[type]||{bg:'#064e3b',color:'#6ee7b7',border:'#10b981'};
  t.style.background=c.bg; t.style.color=c.color; t.style.border=`1px solid ${c.border}`; t.style.opacity='1'; t.textContent=msg;
  clearTimeout(t._tmr); t._tmr=setTimeout(()=>{t.style.opacity='0';},3200);
}
...
// FIX: renderComparison called INSIDE the live listener
function renderHub(){
  const ag=$('projectGrid'), cg=$('completedGrid'); if(!ag) return;
  listen(firebase.database().ref('projects'), snap=>{
    ag.innerHTML=''; cg.innerHTML=''; let aC=0,cC=0;
    if(!snap.exists()){
    ...
    renderComparison(all); // ← always called with fresh data
  });
}

function buildProjectCard(id,d,lb,mb,ls,ms,lp,mp,done){
  const lA=kpiAlertClass(lp), mA=kpiAlertClass(mp);
  const badge=done?`<span class="completed-tag">✓ DONE</span>`:`<span class="active-tag">ACTIVE</span>`;
  const actions=done
...
function renderComparison(projects){
  ...
  if(!projects||projects.length<2){el.innerHTML='<p class="empty-hint">Add 2+ projects to see comparison.</p>';return;}
...
function enterProject(id,readOnly=false){
  detachAll(); currentProjectId=id; currentProjectLocked=readOnly;
...
function unlockForEdit(){ $('lockedBanner')?.classList.add('hidden'); currentProjectLocked=false; showToast('Project unlocked','warn'); }

const ALL_TABS=['labor','materials','billing','changeorders','sitelog','suppliers'];
function switchTab(tab){
  ALL_TABS.forEach(t=>{ $(t+'Panel')?.classList.add('hidden'); $('tab_'+t)?.classList.remove('tab-active'); });
