...
(function (root) {
  function createMaterialsFixture() {
    ...
    function ref(path='', query={}) {
      ...
      const api={key:path.split('/').pop(),child:key=>ref(path+'/'+key),orderByChild:field=>ref(path,{...query,field}),equalTo:equal=>ref(path,{...query,equal}),limitToLast:limit=>ref(path,{...query,limit}),once:async(event,cb)=>{const snap=snapshot(path,value());if(cb)cb(snap);return snap},on:(event,cb)=>{const entry={path,cb,read:()=>snapshot(path,value()),last:JSON.stringify(value())};listeners.add(entry);cb(entry.read())},off:(event,cb)=>{for(const entry of listeners)if(entry.path===path&&(!cb||entry.cb===cb))listeners.delete(entry)},set:async value=>{put(path,value);emit()},update:async updates=>{for(const [key,value]of Object.entries(updates))put(path+'/'+key,value);emit()},push:()=>ref(path+'/-sample-'+String(++sequence).padStart(6,'0')),transaction:async updater=>{put(path,updater(read(path)));emit();return{committed:true,snapshot:snapshot(path,value())}}};return api;
    }
    function emit(){for(const entry of listeners){const snap=entry.read();const next=JSON.stringify(snap.val());if(next!==entry.last){entry.last=next;entry.cb(snap)}}}
